...
from lute.term_parent_map.service import \
    import_file, BadImportFileError, \
    export_unknown_terms, export_terms_without_parents
...
def index():
    ...
    if form.validate_on_submit():
        ...
        if text_file:
            ...
            try:
                ...
                msg = f"Imported {language.name} mappings: " + \
    ...
    books = db.session.query(Book). \
        ...
        order_by(asc(Book.title)).all()
    languages = db.session.query(Language). \
        order_by(asc(Language.name)).all()
    return render_template(
        '/term_parent_map/index.html',
        books = books,
        languages = languages,
        form = form
    )
...
def export_book(bookid):
    ...
    return send_file(temp_file_name, as_attachment=True, download_name='unknown_terms.txt')
...
def export_language(languageid):
    ...
    return send_file(temp_file_name, as_attachment=True, download_name='terms_without_parents.txt')
