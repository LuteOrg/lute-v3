...
def test_save_update_existing(app_context, new_book, repo):
    ...
    b = repo.add(new_book)
    repo.commit()
    assert_sql_result("select BkTitle from books where BkTitle = 'HELLO'", [ "HELLO" ], 'Saved')
    ...
    assert_sql_result("select BkTitle from books where BkTitle = 'CHANGED'", [ "CHANGED" ], 'ok')
    assert_sql_result(
        "select T2Text from tags2 order by T2Text",
        [ 'new1', 'new2', 'tag1', 'tag2' ]
    )
